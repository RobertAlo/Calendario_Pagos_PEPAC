# -*- coding: utf-8 -*-
"""
Editor Calendario FEAGA/FEADER (v0.8.1)
- Calendario anual desplazable con días marcados si hay pagos.
- Mini-frame "Pagos en la fecha seleccionada" que SIEMPRE se actualiza.
- Botones: Actualizar pagos (web), Importar circulares (PDF), Pagos de hoy.
- Aviso superior si hoy hay pagos FEAGA (anticipos/saldos).
- Heurística FEAGA integrada + scrapers/ingestor opcionales.
    ...
    def get_for_day(self, dt: date):
        ...
        if (start_sal <= dt <= date(campaign_year,12,31)) or (date(campaign_year+1,1,1) <= dt <= end_sal):
            out.append({"tipo":"Saldo ayudas directas","fondo":"FEAGA",
                        ...
                        "fuente":"Heurística FEGA"})
        # Fila informativa cuando no hay ventanas FEAGA
        if not out:
...
class YearCalendarFrame(ttk.Frame):
    ...
    def _controls(self):
        top=ttk.Frame(self); top.grid(row=0,column=0,sticky="ew",pady=(0,4)); top.columnconfigure(5,weight=1)
        ...
        spin=ttk.Spinbox(top,from_=1900,to=2100,width=6,textvariable=self.year_var,justify="center",command=lambda:self._set(self.year_var.get()))
        spin.grid(row=0,column=1); spin.bind("<Return>",lambda e:self._set(self.year_var.get())); spin.bind("<FocusOut>",lambda e:self._set(self.year_var.get()))
    ...
    def _month(self,parent,year,month):
        ...
        for r,week in enumerate(weeks,start=2):
            for c in range(7):
                ...
                btn.bind("<Button-1>",lambda e,y=year,m=month,d=day:self._click(y,m,d))
    ...
    def go_to_date(self, dt: date):
        self._set(dt.year); self._click(dt.year, dt.month, dt.day)
    def _click(self,y,m,d):
...
class PaymentsInfoFrame(ttk.Frame):
    def __init__(self, master):
        ...
        for c,(w,anc) in {"tipo":(180,"w"),"fondo":(80,"w"),"detalle":(600,"w"),"fuente":(220,"w")}.items():
...
class CalendarioFEAGA_FEADERFrame(tk.Frame):
    ...
    def _build_dataset(self):
        data = {
            ...
            "Ayuda FEAGA":[
                "No","Sí (Solicitud Única)","Sí","Sí","Sí (Fecha límite Solicitud Única)","Sí","Sí","Sí","Sí",
                "Sí (Pagos anticipados de ayudas directas)","Sí","Sí (Continuación de pagos)","Sí (Pagos finales de ayudas directas)","No"
            ],
            "Ayuda FEADER":[
                "No","Sí (algunas medidas FEADER incluidas en la Solicitud Única)","Sí","Sí","Sí","Sí",
                "Sí (Controles en medidas FEADER)","Sí","Sí","Sí (En medidas que lo contemplen)","Sí",
                "Sí (Proyectos con ejecución anual)","Sí (Dependiendo del proyecto y convocatoria)",
                "Sí (Medidas de desarrollo rural y convocatorias específicas)"
            ]
        }
    ...
    def _build_ui(self):
        ...
        # Barra aviso (oculta por defecto)
        ...
        ttk.Label(self, text="Calendario FEAGA / FEADER – Editor y pagos (v0.8.1)",
        ...
        # IZQUIERDA: toolbar + tabla
        left=ttk.Frame(split); split.add(left, weight=3)
        toolbar=ttk.Frame(left); toolbar.grid(row=0,column=0,columnspan=2,sticky="ew",pady=(0,4))
        ...
        for c in self.columns:
            ...
            self.tree.column(c,width=(150 if c!="Actividad" else 450),anchor="w",stretch=True)
        ...
        self.activity_text=tk.Text(tf,wrap="word"); self.activity_text.grid(row=0,column=0,sticky="nsew")
        ...
        act_ys.grid(row=0,column=1,sticky="ns"); act_xs.grid(row=1,column=0,sticky="ew")
        tf.rowconfigure(0,weight=1); tf.columnconfigure(0,weight=1)
        btns=ttk.Frame(tab_act); btns.grid(row=2,column=0,columnspan=3,sticky="ew",pady=(8,0))
        ...
        self.pay_frame=PaymentsInfoFrame(v_split); v_split.add(self.pay_frame,weight=2)
        try:
    ...
    def _update_today_banner(self):
        ...
        if items:
            tipos=", ".join(sorted({i["tipo"] for i in items if not i["tipo"].startswith("Sin pagos")}))
            msg = f"Hoy ({date.today().strftime('%d/%m/%Y')}) hay pagos FEAGA: {tipos}."
            self.alert_msg.config(text=msg); self.alert_bar.pack(fill="x", padx=10, pady=(10,0))
        else:
            self.alert_bar.pack_forget()
    ...
    def _open_today(self):
        """Ir a Calendario y, además, cargar explícitamente los pagos de HOY."""
        self._select_calendar_tab()
        today=date.today()
        self.yearcal.go_to_date(today)              # sitúa el calendario…
        self._on_calendar_click(today.year, today.month, today.day)  # …y garantiza la carga en el mini-frame
    ...
    def _on_calendar_click(self,y,m,d):
        """SIEMPRE refresca el mini-frame, incluso si no hay ventanas -> muestra 'Sin pagos…'."""
        dt=date(y,m,d)
        items = self.index.get_day(dt) + self.heur.get_for_day(dt)  # ya incluye la fila "Sin pagos…" si aplica
        # dedup
        seen=set(); uniq=[]
        for it in items:
            key=(it["tipo"],it["fondo"],it["detalle"])
    ...
    def edit_selected_modal(self):
        it=self._get_selected_item()
        if not it: messagebox.showinfo("Editar fila","Selecciona primero una fila."); return
        ...
        def ok():
            new=[e_mes.get().strip(), t_act.get("1.0","end").rstrip("\n"), e_feaga.get().strip(), e_feader.get().strip()]
            self.tree.item(it,values=new); self._on_select(); win.destroy()
        ttk.Button(btns,text="Aceptar",command=ok).grid(row=0,column=0,padx=5)
    ...
    def _on_double_click_cell(self,event):
        it=self.tree.identify_row(event.y); col=self.tree.identify_column(event.x)
        if not it or not col: return
        idx=int(col.replace("#",""))-1; name=self.columns[idx]; vals=list(self.tree.item(it,"values"))
        if name=="Actividad": self.edit_selected_modal(); return
        x,y,w,h=self.tree.bbox(it,col)
        top=tk.Toplevel(self); top.overrideredirect(True); top.geometry(f"{w}x{h}+{self.tree.winfo_rootx()+x}+{self.tree.winfo_rooty()+y}")
        e=ttk.Entry(top); e.insert(0, vals[idx]); e.select_range(0,'end'); e.focus(); e.pack(fill="both", expand=True)
        def save(_=None):
            vals[idx]=e.get(); self.tree.item(it,values=vals); top.destroy(); self._on_select()
        e.bind("<Return>",save); e.bind("<Escape>",lambda _ :top.destroy()); e.bind("<FocusOut>",save)
    def apply_activity_to_selected(self):
        it=self._get_selected_item()
        if not it: messagebox.showinfo("Aplicar","Selecciona primero una fila."); return
        vals=list(self.tree.item(it,"values")); vals[1]=self.activity_text.get("1.0","end").rstrip("\n")
        self.tree.item(it,values=vals); messagebox.showinfo("Aplicar","Actividad actualizada.")
    def add_row(self):
    ...
    def delete_selected(self):
        it=self._get_selected_item()
        if not it: messagebox.showinfo("Eliminar fila","Selecciona primero una fila."); return
    ...
    def export_to_excel(self):
        ...
        try:
            with pd.ExcelWriter(path,engine='xlsxwriter') as w:
                ...
                wb=w.book; ws=w.sheets['Calendario']
                fmt=wb.add_format({'text_wrap':True,'valign':'top'})
                idx=df.columns.get_loc('Actividad'); col=self._excel_col(idx)
    ...
    def reset_data(self):
        if messagebox.askyesno("Restablecer","¿Restablecer los datos originales?"):
    ...
    @staticmethod
    def _excel_col(idx0:int)->str:
        idx=idx0; s=""
        while True:
            idx,rem=divmod(idx,26); s=chr(ord('A')+rem)+s
            if idx==0: break
            idx-=1
        return s
...
def main():
    root=tk.Tk()
    root.title("Editor Calendario FEAGA/FEADER (v0.8.1)")
